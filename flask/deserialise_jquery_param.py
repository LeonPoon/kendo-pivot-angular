...
def deserialise_jquery_param(args, re=regex.compile('^([A-Za-z_][A-Za-z0-9_]*)(?:\\[((?:0|[1-9][0-9]*)|(?:[A-Za-z_][A-Za-z0-9_]*)*)\\])*$'), val_map={'true': True, 'false': False}):
    obj = {}
    for k, v in args:
        match = re.match(k)
        if match:
...
class DeserialiseJQueryTest(unittest.TestCase):
    ...
    def do_it(self, original, params, old, query_string, msg):
        cls = self.__class__
        d = deserialise_jquery_param((reduce(lambda a, b: a.replace(*b), self.urlencoded.items(), arg).split('=') for arg in query_string.split('&')) if query_string else (), val_map={})
